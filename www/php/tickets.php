    ...
    class Tickets extends DB
    {
        ...
        public function getUsersTickets($userID, $flagged = false)
        {
            ...
            return $result;
                ...
                public function Ascending($flagged = false)
                //function which sorts the tickets in Ascending order
        {
            $query = "SELECT tickets.id, subject, content, closed, user_id, created_at, updated_at, closed_at, accounts.name
                     FROM accounts
                     Inner JOIN tickets ON accounts.id=tickets.user_id
                     ORDER BY tickets.subject,accounts.name";
             if($flagged){
           $query = "SELECT tickets.id,subject, content, closed, user_id, created_at, updated_at, closed_at,accounts.name
                     FROM accounts
                     Inner JOIN tickets ON accounts.id=tickets.user_id
            ...
            {   
                ...
                $stmt->bind_result($id, $subject, $content, $closed, $userID, $createdAt, $updatedAt, $closedAt,$user_name);
                ...
                {                    
                    while($stmt->fetch())
                    {
                        array_push($result, [$id, $subject, $content, $closed, $userID, $createdAt, $updatedAt, $closedAt, $user_name]);

                    }
                   
                }else {echo"error with while statement".mysqli_error($stmt);}
                $stmt->close();
                
            }
            else {echo"error with connection".mysqli_error($stmt);}
            $this->close();
            return $result;
}
        
        public function Descending($flagged = false)
        //function which sorts the tickets in Descending order
        {
            $query = "SELECT tickets.id, subject, content, closed, user_id, created_at, updated_at, closed_at, accounts.name
                     FROM accounts
                     Inner JOIN tickets ON accounts.id=tickets.user_id
                     ORDER BY tickets.subject DESC,accounts.name DESC";
             if($flagged){
           $query = "SELECT tickets.id,subject, content, closed, user_id, created_at, updated_at, closed_at,accounts.name
                     FROM accounts
                     Inner JOIN tickets ON accounts.id=tickets.user_id
